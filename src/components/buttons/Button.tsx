...
const Button = (props: Props & MotionProps) => {
  ...
  const classes = `${size === 'sm'
      ? 'p-3 px-4 text-sm border-[1.5px] min-h-[44px]'
      : 'text-sm p-4 px-6 border-2 min-h-[48px]'
    } block ${center ? 'mx-auto' : ''
    } w-fit font-mono capitalize rounded border-accent text-accent hover:bg-accent-light focus:outline-none focus:bg-accent-light active:scale-95 duration-150 cursor-pointer touch-manipulation ${className}`;
